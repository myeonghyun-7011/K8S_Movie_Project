...
const MovieTheaters = () => {
  ...
  const initMap = () => {
    ...
    // Add markers for theaters if theaters data is available
    if (theaters.length > 0) {
      theaters.forEach(theater => {
        const marker = new window.google.maps.Marker({
          ...
          title: theater.name,
          icon: {
            url: 'http://maps.google.com/mapfiles/ms/icons/red-dot.png' // 빨간색 마커 아이콘 URL
          }
        });
